...
export const metodos = {
    ...
    renderCartProduct: function() {
        ...
        variables.carrito.data.forEach((item, i) => {
            ...
            variables.carrito.contenedor.itemConten.price.innerHTML = `<div><h4 class="cart-price" data='${item.id}'>${item.price * item.cantidad}</h4></div>`
            ...
            variables.carrito.contenedor.itemConten.cantidadTotal = document.createElement('h6')
            variables.carrito.contenedor.itemConten.cantidadTotal.textContent = item.cantidad
            variables.carrito.contenedor.itemConten.cantidadTotal.setAttribute('data', item.id)
            variables.carrito.contenedor.itemConten.rest = document.createElement("span")
            variables.carrito.contenedor.itemConten.rest.innerText = '➖'
            variables.carrito.contenedor.itemConten.rest.setAttribute('class', 'btn rest')

            variables.carrito.contenedor.itemConten.rest.addEventListener('click', () => this.modCart(i, 'resta', item.id), false)
                // variables.carrito.contenedor.itemConten.modCart.rest.addEventListener('click', () => this.modCart(i), false)

            variables.carrito.contenedor.itemConten.plus = document.createElement("span")
            variables.carrito.contenedor.itemConten.plus.setAttribute('class', 'btn plus')
            variables.carrito.contenedor.itemConten.plus.textContent = '➕'
            variables.carrito.contenedor.itemConten.plus.addEventListener('click', () => this.modCart(i, 'suma', item.id), false)
                // add botn de boorado
            variables.carrito.contenedor.itemConten.deleteItem = document.createElement("span")
            variables.carrito.contenedor.itemConten.deleteItem.setAttribute('class', 'btn delete')

            variables.carrito.contenedor.itemConten.deleteItem.textContent = '❌'
            variables.carrito.contenedor.itemConten.deleteItem.addEventListener('click', () => this.modCart(i, 'borrar', item.id), false)


            // add al contenedor de las cantidades
            variables.carrito.contenedor.itemConten.cantidad.append(variables.carrito.contenedor.itemConten.rest)
            variables.carrito.contenedor.itemConten.cantidad.append(variables.carrito.contenedor.itemConten.cantidadTotal)
            variables.carrito.contenedor.itemConten.cantidad.append(variables.carrito.contenedor.itemConten.plus)
            variables.carrito.contenedor.itemConten.cantidad.append(variables.carrito.contenedor.itemConten.deleteItem)
        ...
        if (action === 'resta') {
            ...
            variables.carrito.data[element].cantidad--
                if (variables.carrito.data[element].cantidad !== 0) {
                    ...
                    console.log(document.querySelector(`div[data="${id.toString()}"].item`))
    ...
    getTotal: function(data) {
        ...
        return variables.carrito.data.map(price => (price.price * price.cantidad)).reduce((acc, price) => acc + price, 0)
    },
    fetchProducts: async() => {
        try {
